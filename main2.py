...
class hello(QtGui.QMainWindow, GUI.Ui_Form): 
    ...
    def __init__(self):
        ...
        self.files = ["Lowpass.xml", "Highpass.xml", "Notch.xml"]		#the XML files that holds the filtered data
    ...
    def ImportFn(self):
        ...
        self.outputSignal=[] #empty output signal
        ...
        time = (len(samples))/float(FSample) #Calculate files duration in seconds
    ...
    def FunctionSave(self): #the function to save the data after filtering
    	filename = QtGui.QFileDialog.getSaveFileName(self, "Save file", "untitled", ".wav") #gets file name and path
    ...
    def ListenFn(self): #function applies the filter on the signal
        if self.weHaveSample: #checks  if we have data
            ...
            while i < len(samples): #to the last sample
                ...
                cDom = 1 #Demonerator pointer sart point
    ...
    def readFile(self, type): #read xml files function (gets file number in the file list)
        ...
        k = float(myKValue[0].attributes['k'].value) #get gain 
